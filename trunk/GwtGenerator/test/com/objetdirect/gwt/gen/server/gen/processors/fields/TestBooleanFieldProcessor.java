 ...
 */
public class TestBooleanFieldProcessor extends TestProcessor {
	private static final String FIELD_TITLE_VALUE = "Name";
	private static final String FIELD_NAME_VALUE = "name";

	@Test
	public void process() {
		BooleanFieldProcessor pdp = new BooleanFieldProcessor(seamGenerator);
		...
		BooleanField booleanFieldExepected = new BooleanField(FIELD_NAME_VALUE, FIELD_TITLE_VALUE);
		
		pdp.process(objectArgument);
		
		verify(seamGenerator).addBridgeObject(eq(objectArgument), eq(booleanFieldExepected));
	}
}
