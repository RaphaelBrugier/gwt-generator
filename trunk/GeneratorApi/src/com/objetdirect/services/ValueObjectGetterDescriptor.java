...
public class ValueObjectGetterDescriptor implements DataAccessServiceElement {
	
	public ValueObjectGetterDescriptor(ValueObjectDescriptor valueObject, String ... loadPathes) {
	...
	public void prepare() {
		...
		getter = buildValueObjectGetter();
		getOwner().publishMethod(getter);
	}
	
	protected MethodDescriptor buildValueObjectGetter() {
		MethodDescriptor getter = new MethodDescriptor();
		getter.init(
			owner.getImplementationDescriptor(), 
			valueObject.getClassDescriptor(), 
			new StandardNameMaker("get", null, valueObject.getEntityDescriptor().getClassDescriptor()));
		String paramName = NamingUtil.toMember(valueObject.getEntityDescriptor().getName())+"Id";
		getter.addModifier("public").addParameter(TypeDescriptor.rInt, paramName);
		getter.setContent(defaultPattern);
		String entityInstanceName = NamingUtil.toMember(valueObject.getEntityDescriptor().getClassName());
		getter.replace("EntityClass", valueObject.getEntityDescriptor().getClassDescriptor());
		getter.replace("ValueObjectClass", valueObject.getClassDescriptor());
		getter.replace("entityInstance", entityInstanceName);
		getter.replace("Mapper", com.objetdirect.entities.Frmk.Mapper);
		getter.replace("EntityNotFoundException", JavaxPersistence.EntityNotFoundException);
		getter.replace("entityManager", getOwner().getEntityManager());
		getter.replace("paramId", paramName);
		processPathStore(getter, valueObject, entityInstanceName, loadPathes, "/// load collections here");
		return getter;
	}
	...
	static final String[] defaultPattern = {
		"EntityClass entityInstance = entityManager.find(EntityClass.class, paramId);",
